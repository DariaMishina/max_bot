...
QUERY4="
SELECT 
    COUNT(CASE WHEN ub.free_divinations_remaining = 0 AND ub.paid_divinations_remaining = 0 AND (ub.unlimited_until IS NULL OR ub.unlimited_until < NOW()) THEN 1 END) as \"Полностью нулевой баланс\",
    COUNT(CASE WHEN ub.free_divinations_remaining = 0 AND ub.paid_divinations_remaining > 0 THEN 1 END) as \"Только платные остались\",
    COUNT(CASE WHEN ub.free_divinations_remaining > 0 AND ub.paid_divinations_remaining = 0 THEN 1 END) as \"Только бесплатные остались\",
    COUNT(CASE WHEN ub.unlimited_until IS NOT NULL AND ub.unlimited_until > NOW() THEN 1 END) as \"С активным безлимитом\",
    COUNT(CASE WHEN ub.free_divinations_remaining = 1 THEN 1 END) as \"Осталось 1 бесплатное\",
    COUNT(CASE WHEN ub.free_divinations_remaining = 2 THEN 1 END) as \"Осталось 2 бесплатных\",
    COUNT(CASE WHEN ub.free_divinations_remaining = 3 THEN 1 END) as \"Осталось 3 бесплатных\"
