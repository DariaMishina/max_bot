...
WITH today_users AS (
    ...
    FROM max_users
    WHERE user_id NOT IN $EXCLUDE_USERS
        AND ((created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date <= $MSK_TODAY
),
yesterday_users AS (
    SELECT COUNT(DISTINCT user_id) as count
    FROM max_users
    WHERE user_id NOT IN $EXCLUDE_USERS
        AND ((created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date <= $MSK_YESTERDAY
),
day_before_users AS (
    SELECT COUNT(DISTINCT user_id) as count
    FROM max_users
    WHERE user_id NOT IN $EXCLUDE_USERS
        AND ((created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date <= $MSK_BEFORE_YESTERDAY
)
SELECT 
...
WITH registrations AS (
    SELECT 
        ...
        CASE 
            ...
            ELSE 'Органика' 
        END as client_type,
        ((c.conversion_datetime AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date as reg_date
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN reg_date = $MSK_TODAY THEN user_id END) as \"Прирост за сегодня\",
    COUNT(DISTINCT CASE WHEN reg_date = $MSK_YESTERDAY THEN user_id END) as \"Прирост за вчера\",
...
SELECT 
    ...
    TO_CHAR((u.created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY', 'DD.MM.YY HH24:MI') as \"Регистрация (МСК)\",
    CASE 
...
QUERY1_DAILY_GROWTH="
SELECT 
    TO_CHAR(((created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date, 'DD.MM.YY') as \"Дата (МСК)\",
    COUNT(*) as \"Новых\",
...
FROM max_users
WHERE user_id NOT IN $EXCLUDE_USERS
    AND (created_at AT TIME ZONE '$REPORT_TZ_STORED') >= ((($MSK_TODAY) - 14) AT TIME ZONE '$REPORT_TZ_DISPLAY')
GROUP BY ((created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date
ORDER BY ((created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date DESC;
"
...
WITH user_periods AS (
    ...
    SELECT 
        user_id,
        CASE 
            WHEN ((conversion_datetime AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date <= '2026-02-02'::date THEN 'До 02.02.26 (вкл.)'
            ELSE 'После 02.02.26'
        END as period
    FROM max_conversions
...
WITH user_sources AS (
    ...
    WHERE c.conversion_type = 'registration'
        AND c.user_id NOT IN $EXCLUDE_USERS
        AND ((c.conversion_datetime AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date > '2026-02-02'::date
),
funnel AS (
    SELECT 
        us.source_type,
...
QUERY7_TOTAL="
SELECT 
    TO_CHAR(((d.created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date, 'DD.MM.YY') as \"Дата (МСК)\",
...
WHERE d.user_id NOT IN $EXCLUDE_USERS
    AND (d.created_at AT TIME ZONE '$REPORT_TZ_STORED') >= ((($MSK_TODAY) - 2) AT TIME ZONE '$REPORT_TZ_DISPLAY')
GROUP BY ((d.created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date
ORDER BY ((d.created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date DESC;
"
...
GROUP BY ((d.created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date,
    CASE 
        ...
        ELSE 'Органика'
    END
ORDER BY ((d.created_at AT TIME ZONE '$REPORT_TZ_STORED') AT TIME ZONE '$REPORT_TZ_DISPLAY')::date DESC, \"Источник\";
"
...
echo ""
echo "Дата и время выполнения (МСК): $(TZ=Europe/Moscow date '+%Y-%m-%d %H:%M:%S %Z')"
echo ""
