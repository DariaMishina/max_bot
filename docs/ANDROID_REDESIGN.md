...
Редизайн выполняется поверх работающего MVP. Гостевая сессия, API, история, лимиты и безопасный повтор запроса не меняются. Сначала выбирается направление, затем реализуется вертикальный срез «онбординг → вопрос → результат» и проверяется на телефоне.

## Концепции

### 1. Лунный атлас — рекомендуется
...
1. Внедрить палитру, типографику и базовые компоненты.
2. Согласовать тёмные системные панели.
3. Обновить онбординг и убрать устаревший текст.
4. Переработать вопрос и выбор режима; при ручном выборе убирать фокус с поля.
5. Обновить рубашку, сетку, номера и прогресс выбора.
6. Переработать результат, ожидание, повтор и уточнения.
7. Проверить ширину 360/412 dp, `fontScale` 1.0/1.3/2.0, клавиатуру и TalkBack.
8. Выполнить `scripts/check_android.sh` и телефонный чек-лист.
9. После подтверждения распространить систему на историю, магазин и профиль.
